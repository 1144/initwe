...
async function work(initData) {
    ...
    FS.readdirSync(dir).forEach(filename => {
      ...
      if (relPath !== initOptionYML
        ...
        if (FS.statSync(filePath).isFile()) {
          ...
          Util.makeDir(Path.join(dest, newPath))
          handleDir(filePath)
        }
      }
    })
  ...
  const handleZipFiles = () => {
    for (const filePath in files) {
      const relPath = Util.trimSlash(filePath.slice(srcLen)) // 相对项目根的路径
      if (!relPath) {
        continue
      }
...
async function handleFrom(initData) {
  ...
  if (localPath) {
    ...
    return {
      absSrc: Util.trimSlash(Path.join(localPath, _src)), // src 的绝对路径
      files: null,
    }
  }
...
async function handleFile(file, dest, newPath, workData) {
  ...
  if (FS.existsSync(path)) {
    ...
    if (i > 0) {
      Util.makeDir(Path.join(dest, newPath.slice(0, i)))
    }
